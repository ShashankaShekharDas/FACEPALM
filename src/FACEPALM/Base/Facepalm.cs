...
{
    private readonly GenericPostgresDatabaseHelper<CredentialStore> _credentialStoreProvider = new GenericPostgresDatabaseHelper<CredentialStore>();
    
    private static List<string> GetAllFilesInDirectory(string directory) => Directory.GetFiles(directory).ToList();
    ...
    {
        var validProviders = await _credentialStoreProvider.SearchRows([new WhereClause("maxsizeinbytes - usedsizeinbytes", "5000000", DatabaseOperator.GreaterThanOrEqual)], CredentialStore.Deserialize);
    ...
    public async Task<bool> UploadFile(string path)
    {
        ...
        if (validProviders.Count == 0) throw new NoValidProviderException("No valid providers found");
